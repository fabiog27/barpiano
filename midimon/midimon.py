...
notenames = ["C", "Cs", "C#", "Dk", "D", "Ds", "D#", "Ek", "E", "Es", "F", "Fs", "F#", "Gk", "G", "Gs", "G#", "Ak", "A",
             "As", "A#", "Bk", "B", "Bs"]
...
def MidiCallback(mididev, message, time_stamp):
    ...
    if status in systemmsg:
        ...
        if messagetype == 9:  # note off/on
            ...
            note_name = midinote2notename(data1, 1)
            spec = "note %d=%s, pressure=%d" % (data1, note_name["full_name"], data2)
...
try:
    ...
    in_ports = rtmidi2.get_in_ports()
    out_ports = rtmidi2.get_out_ports()
print_sensing("in", in_ports)
print_sensing("out", out_ports)
